classdef NewMultiPoolTofftsGammaVIF_Edit < HPKinetics.MultiPoolToffts
...
Mz(:,1) = M0.*cos(FaList(:,1)); %first step -> 2 numbers? 2 bounds? z is input i guess
             ...
             %v1 = Mxy %should be the "boundary condition" but it may not be used
              ...
              if MDAnderson2023REU
                  MDfunc = @(y) 6.684 * exp(-((y - 12.97)/8.074).^2) %Manually CF tooled to get this from VIF array values
                  for i = 2:N
